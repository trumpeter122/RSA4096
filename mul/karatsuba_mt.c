...
#define KARATSUBA_MT_MIN_DIGITS 128
...
static void karatsuba_mul_serial(bn_t *a, bn_t *b, bn_t *c, uint32_t digits);
static void karatsuba_mul_parallel(bn_t *a, bn_t *b, bn_t *c, uint32_t digits);
static void *karatsuba_thread(void *arg);
...
void mul_bn_mul(bn_t *a, bn_t *b, bn_t *c, uint32_t digits)
{
    ...
    karatsuba_mul_parallel(tt, bb, cc, padded);
...
static void karatsuba_mul_parallel(bn_t *a, bn_t *b, bn_t *c, uint32_t digits)
{
    ...
    if(digits < KARATSUBA_MT_MIN_DIGITS) {
        karatsuba_mul_serial(a, b, c, digits);
        return;
    }
    ...
    threaded = (pthread_create(&thread, NULL, karatsuba_thread, &worker) == 0);
    karatsuba_mul_serial(z0, b, c, half);
    if(threaded) {
        pthread_join(thread, NULL);
    } else {
        karatsuba_mul_serial(z2, b+half, c+half, half);
    }
...
static void *karatsuba_thread(void *arg)
{
    karatsuba_worker_t *worker = (karatsuba_worker_t *)arg;

    karatsuba_mul_serial(worker->a, worker->b, worker->c, worker->digits);
    return NULL;
}
